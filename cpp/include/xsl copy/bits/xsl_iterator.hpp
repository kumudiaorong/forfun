...
public:
  ...
  constexpr pointer_wrapper(T *ptr)
    : Ptr(ptr) {}
  template <class Other, ts::enable<ts::tp::is_constructible<T *, Other *>> = 0>
...
public:
  ...
  template <class Other, ts::enable<ts::tp::is_constructible<_Iter, Other>> = 0>
...
inline constexpr bool has_iterator_operator =
  ts::tp::has_opeartor_pre_increment<UCIter> && ts::tp::has_opeartor_pre_increment<UCIter>
  && ts::tp::has_opeartor_pre_increment<UCIter>;
//
template <class UCIter>
inline constexpr bool is_iter = ts::is_pointer<UCIter> || has_iterator_operator<UCIter>;
//
//
template <class UCIter>
constexpr bool is_forward_iter =
  is_iter<UCIter> && ts::tp::is_existing<forward_iter, typename iter_traits<UCIter>::iter_category>;
//
template <class UCIter>
constexpr bool is_reverse_iter =
  is_iter<UCIter> && ts::tp::is_existing<reverse_iter, typename iter_traits<UCIter>::iter_category>;
//
template <class UCIter>
constexpr bool is_bilateral_iter =
  is_iter<UCIter> && ts::tp::is_existing<bilateral_iter, typename iter_traits<UCIter>::iter_category>;
//
template <class UCIter>
constexpr bool is_random_iter =
  is_iter<UCIter> && ts::tp::is_existing<random_iter, typename iter_traits<UCIter>::iter_category>;
//
...
constexpr CIter get_tar_iter(CIter&& Iter, SizeType Count) {
  if constexpr(ts::tp::has_opeartor_addition<ts::rm_vr<CIter>, SizeType>)
    return Iter + Count;
  else if constexpr(ts::tp::has_opeartor_addition_assignment<ts::rm_vr<CIter>, SizeType>)
    return Iter += Count;
  else {
    while(Count-- > 0)
      ++Iter;
