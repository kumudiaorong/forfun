...
namespace batch {
  ...
  constexpr int_8 compare(CIter1 first1, CIter2 first2, size_t size) {
    ...
    if constexpr(ts::is::basic_type<val_type> && ts::is::pointer<CIter1> && ts::is::pointer<CIter2>)
  ...
  constexpr size_t length(
    CIter first, const typename iter_traits<CIter>::val_type& end = typename iter_traits<CIter>::val_type{}) {
    size_t count{};
    while(*first != end)
      ++first, ++count;
    return count;
