...
namespace xsl {
  ...
  struct pair {
    ...
    constexpr pair(_First_&& _first, _Second_&& _second)
      : first(forward<_First_>(_first))
      , second(forward<_Second_>(_second)) {
    }
  };
  ...
  public:
    ...
    constexpr set_pair& _1(Args&&...args) {
      construct_at(const_cast<ts::rm::cst<typename _Pair::first_type> *>(addr(Pair->first)), forward<Args>(args)...);
      return *this;
    }
    template <class... Args>
    constexpr set_pair& _2(Args&&...args) {
      construct_at(const_cast<ts::rm::cst<typename _Pair::second_type> *>(addr(Pair->second)), forward<Args>(args)...);
      return *this;
    }
  };
}  // namespace xsl
