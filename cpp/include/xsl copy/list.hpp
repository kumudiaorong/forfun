...
#ifndef XSL_LIST
#define XSL_LIST
#include"bits/xsl_utility.h"
#include"bits/xsl_container.h"
#include"bits/xsl_batch.h"
#include"bits/xsl_allocator.h"
#include"bits/xsl_iterator.h"
#include"bits/xsl_pair.h"
XSL_BEGIN
//
...
public:
	using val_type = _Val;
	//neighbor
	constexpr list_node():Val(), Prev(), Next() { }
	//
	template<typename Arg, typename...Args, enable<!is_same<list_node, rm_cvr<Arg>>> = 0>
	constexpr list_node(Arg&& param, Args&&...params) : Val(forward<Arg>(param), forward<Args>(params)...), Prev(), Next() {}
	//
	constexpr list_node(const list_node& Ano) : Val(Ano.Val), Prev(Ano.Prev), Next(Ano.Next) { }
	//
	val_type Val;
	list_node* Prev, * Next;
};
...
public:
  // clang-format on
	...
	typedef list_iter 						mnr_type;
  // clang-format on
	constexpr list_iter(node_type* ptr):Ptr(ptr) {}
	//
...
public:
  // clang-format on
	...
	typedef _Alloc::template rebind_alloc<node_type>	alloc_type;
...
// #define XSL_GROUP_TO_SAME_MCTR_FOR_CONSTRUCTOR(ARG0,ARG1)				\
	...
	constexpr list(Unchecked_Iter First, ARG0) : list() {				\
		...
		// XSL_GROUP_TO_SAME_MCTR(2, XSL_GROUP_TO_SAME_MCTR_FOR_CONSTRUCTOR, Unchecked_Iter Last, size_type Count, get_mnr_iter(Last), Count)
// #undef XSL_GROUP_TO_SAME_MCTR_FOR_CONSTRUCTOR
		//
// #else
	//
	constexpr list():Alc(), Head(Alc.allocate(sizeof(node_type))), Size() {
		Head->Prev = Head->Next = Head;
	}
	constexpr list(size_type Count) : list() {
		this->insert(Head, creator{ Count });
	}
	//
	constexpr list(size_type Count, const val_type& Val) : list() {
		this->insert(Head, creator{ Count, Val });
	}
	//
	template<class Unchecked_Iter, enable<is_forward_iter<Unchecked_Iter>> = 0>
	constexpr list(Unchecked_Iter First, Unchecked_Iter Last) : list() {
		this->insert(Head, creator{ get_mnr_iter(First), get_mnr_iter(Last) });
	}
	//
	template<class Unchecked_Iter, enable<is_iter<Unchecked_Iter>> = 0>
	constexpr list(Unchecked_Iter First, size_type Count) : list() {
...
// #endif // _DEBUG
	...
	constexpr list& operator=(const list& Ano) {
		return this->assign(creator{ iter{ Ano.Head->Next }, Ano.Size });
	}
	//
// #ifndef _DEBUG
	//
...
// #define XSL_GROUP_TO_SAME_MCTR_FOR_ASSIGN(ARG0,ARG1)					\
	...
	constexpr list& assign(Unchecked_Iter First, ARG0) {				\
		...
		// XSL_GROUP_TO_SAME_MCTR(2, XSL_GROUP_TO_SAME_MCTR_FOR_ASSIGN, Unchecked_Iter Last, size_type Count, get_mnr_iter(Last), Count);
// #undef XSL_GROUP_TO_SAME_MCTR_FOR_ASSIGN
	//
// #else
	//
	constexpr list& assign(size_type Count) {
		return assign(creator{ Count });
	}
	//
	constexpr list& assign(size_type Count, const val_type& Val) {
		return assign(creator{ Count, Val });
	}
	//
	template<class Unchecked_Iter, enable<is_iter<Unchecked_Iter>> = 0>
...
// #endif // _DEBUG
	...
	constexpr list& assign(const list& Ano) {
		return assign(creator{ iter{ Ano.Head->Next }, Ano.Size });
	}
	//
// #ifndef _DEBUG
	//
// #define	XSL_FILL_OPT_FOR_CONST(CONST)		\
	...
	constexpr CONST riter rbegin()CONST {	\
		XSL_EMPTY_CHECK(this->invalid());	\
		return GPtr_T{ Head->Prev };		\
	}										\
	constexpr CONST riter rend()CONST {		\
		XSL_EMPTY_CHECK(this->invalid());	\
		return GPtr_T{ Head };				\
	}
	// XSL_FILL_OPT(1, XSL_FILL_OPT_FOR_CONST, const)
// #undef XSL_FILL_OPT_FOR_CONST
		//
// #else
...
// #define XSL_GROUP_TO_SAME_MCTR_FOR_INSERT(ARG0,ARG1)						\
	...
	constexpr void insert(citer where, Unchecked_Iter First, ARG0) {	\
		...
		// XSL_GROUP_TO_SAME_MCTR(2, XSL_GROUP_TO_SAME_MCTR_FOR_INSERT, Unchecked_Iter Last, size_type Count, get_mnr_iter(Last), Count)
// #undef XSL_GROUP_TO_SAME_MCTR_FOR_INSERT
		//
// #else
	//
	constexpr void insert(citer where, size_type Count) {
		this->insert(where.Ptr, creator{ Count });
	}
	//
...
// #endif // _DEBUG
	...
	//
	constexpr void remove(citer dest) {
		this->erase(condition{ iter{dest.Ptr}, 1 });
	}
	//
	constexpr void erase(citer first, citer last) {
		this->erase(condition{ iter{first.Ptr}, iter{last.Ptr} });
	}
	//
	constexpr void erase(citer first, size_type count) {
		this->erase(condition{ iter{first.Ptr}, count });
	}
	//
	...
	constexpr void pop_back() {
		this->erase(condition{ iter{Head->Prev}, 1 });
	}
	//
	template<typename...Args>
	...
	constexpr void pop_front() {
		this->erase(condition{ iter{Head->Next}, 1 });
	}
	//
// #ifndef _DEBUG
//
// #define XSL_FILL_OPT_FOR_RESIZE(ARG0,ARG1)									\
	constexpr void resize(size_type NewSize XSL_ADD_COMMA(ARG0)) {				\
		if (Size < NewSize)													\
			this->Insert(Head, creator(NewSize - Size XSL_ADD_COMMA(ARG1)));\
		else if (Size > NewSize) {											\
			node_type* where = Head;											\
			size_type Count = Size - NewSize;									\
			while (NewSize < Size)											\
				where = where->Prev, ++NewSize;								\
			this->Erase(where, Count);										\
		}																	\
	}
	// XSL_FILL_OPT(2, XSL_FILL_OPT_FOR_RESIZE, const val_type& Val, Val)
// #undef XSL_FILL_OPT_FOR_RESIZE
		//
// #else
//
...
protected:
	...
	size_type Size;         // size
	//
	template<class Creator, enable<is_xsl_creater<rm_cvr<Creator>>> = 0>
	constexpr list& assign(Creator&& ctor) {
		if (this->invalid()) {
			...
			size_type size = Size;
			if (ctor)
				insert(Head, as_rreference(ctor));
			this->erase(condition{ iter{Head->Next}, size });
		}
		return *this;
	}
