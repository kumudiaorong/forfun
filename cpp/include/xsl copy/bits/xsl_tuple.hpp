...
public:
  ...
  template <typename T>
  constexpr T get() {
    if constexpr(ts::is_same<val_type, T>)
      return Val;
    else
  ...
  constexpr decltype(auto) get() {
    if constexpr(Id == 0)
      return static_cast<ts::conditional_apply<!ts::is_rref<val_type>, ts::as_lref, val_type>>(Val);
    else
  ...
  constexpr decltype(auto) get() const {
    if constexpr(Id == 0)
      return static_cast<ts::conditional_apply<!ts::is_rref<val_type>, ts::as_lref, ts::as_c<val_type>>>(Val);
    else
      return base_type::template get<Id - 1>();
