...
#ifndef XSL_SEQLIST
#define XSL_SEQLIST
#include"bits/xsl_seqlist.h"
XSL_BEGIN


template<typename _Val, typename _SizeType = uint_32, typename _Alloc = unfixed_block_allocator<_Val, _SizeType>>
class SeqList :base_seqlist<_Val, _SizeType, _Alloc> {
	//
public:
	using Base = base_seqlist<_Val, _SizeType, _Alloc>;
	using Val_T = Base::Val_T;
	using Alloc_T = Base::Alloc_T;
	using Addr_T = Base::Addr_T;
	using Size_T = Base::Size_T;
	using GPtr_T = Base::GPtr_T;
	using Iter = Base::Iter;
	using RIter = Base::RIter;
	//using Base_iter = typename Iter::Base;
	using Base_iter = Base::Base_iter;
	//
	using Base::Base;
	using Base::operator=;
	//
	constexpr SeqList& assign(const Val_T* Ptr, Size_T NewSize) {
	...
	//
	constexpr SeqList& assign(const Val_T* Ptr) {
		return assign(Ptr, batch::Len< Size_T>(Ptr));
	}
	//
	constexpr SeqList& assign(SeqList&& Ano) {
		if (Head != Alloc_T::nulladdr) {
			batch::Destroy_n(Alc.addr(Head), Size);
			Alc.free(Head);
		}
	...
	//
	constexpr SeqList& assign(const SeqList& Ano) {
	...
	//
	constexpr SeqList& catenate(const Val_T* Ptr, Size_T Count) {
		Recap_with_moving(Size + Count);
	...
	//
	constexpr SeqList& catenate(const Val_T* Ptr) {
		return catenate(Ptr, batch::Len< Size_T>(Ptr));
	}
	//
	constexpr SeqList& catenate(const SeqList& Ano) {
...
private:
	...
	//
#ifndef _DEBUG
#define XSL_SEQLIST_RECAP_MOVING(CON,COPY)											\
	...
	XSL_SEQLIST_RECAP_MOVING(with, MCopy(Alc.addr(NewHead), Alc.addr(Head), Size * sizeof(Val_T)));
	XSL_SEQLIST_RECAP_MOVING(without, XSL_EMPTR_MACRO);
#undef XSL_SEQLIST_RECAP_MOVING
#else
...
//using string = SeqList<char>;


#ifdef XSL_TEST
void xsl_test_SeqList() {
	TEST_SPLIT_LINE(char);
	TEST_ACT_PRINT(char cstr00[] = "123"; char cstr01[] = "123456");
	TEST_ACT_PRINT(SeqList<char> str00{});
	TEST_RET_PRINT(str00, print);
	TEST_ACT_PRINT(SeqList<char> str01(cstr00));
	TEST_RET_PRINT(str01, print);
	TEST_ACT_PRINT(SeqList<char> str02(cstr01, 6));
	TEST_RET_PRINT(str02, print);
	TEST_ACT_PRINT(SeqList<char> str03{ as_rreference(str01) });
	TEST_RET_PRINT(str03, print);
	TEST_ACT_PRINT(SeqList<char> str04{ str02 });
	TEST_RET_PRINT(str04, print);
	TEST_ACT_PRINT(SeqList<char> str05(3, '1'));
	...
	TEST_ACT_PRINT(SeqList<wchar_t> wstr00{});
	TEST_RET_PRINT(wstr00, print);
	TEST_ACT_PRINT(SeqList<wchar_t> wstr01(cwstr00));
	TEST_RET_PRINT(wstr01, print);
	TEST_ACT_PRINT(SeqList<wchar_t> wstr02(cwstr01, 6));
	TEST_RET_PRINT(wstr02, print);
	TEST_ACT_PRINT(SeqList<wchar_t> wstr03{ as_rreference(wstr01) });
	TEST_RET_PRINT(wstr03, print);
	TEST_ACT_PRINT(SeqList<wchar_t> wstr04{ wstr02 });
	TEST_RET_PRINT(wstr04, print);
	TEST_ACT_PRINT(SeqList<wchar_t> wstr05(3, '1'));
	...
	TEST_ACT_PRINT(SeqList<test_t> ttstr00{});
	TEST_RET_PRINT(ttstr00, print);
	TEST_ACT_PRINT(SeqList<test_t> ttstr01(test_ts + 3));
	TEST_RET_PRINT(ttstr01, print);
	TEST_ACT_PRINT(SeqList<test_t> ttstr02(test_ts, 6));
	TEST_RET_PRINT(ttstr02, print);
	TEST_ACT_PRINT(SeqList<test_t> ttstr03{ as_rreference(ttstr01) });
	TEST_RET_PRINT(ttstr03, print);
	TEST_ACT_PRINT(SeqList<test_t> ttstr04{ ttstr02 });
	TEST_RET_PRINT(ttstr04, print);
	TEST_ACT_PRINT(SeqList<test_t> ttstr05(3, test_ts[0]));
	...
	TEST_ACT_PRINT(SeqList<test_p> tpstr00{});
	TEST_RET_PRINT(tpstr00, print);
	TEST_ACT_PRINT(SeqList<test_p> tpstr01(test_ps, 3));
	TEST_RET_PRINT(tpstr01, print);
	TEST_ACT_PRINT(SeqList<test_p> tpstr02(6, test_ps[0]));
	TEST_RET_PRINT(tpstr02, print);
	TEST_ACT_PRINT(SeqList<test_p> tpstr03{ as_rreference(tpstr01) });
	TEST_RET_PRINT(tpstr03, print);
	TEST_ACT_PRINT(SeqList<test_p> tpstr04{ tpstr02 });
