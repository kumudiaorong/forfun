...
public:
	...
	using Iter = random_forward_bilateral_iterator<view>;
	using RIter = random_reverse_bilateral_iterator<view>;
	using Base_Iter = Iter::Base;
	//
	constexpr view(const Val_T* Ptr) :view(Ptr, batch::Len<Size_T>(Ptr)) {}
	//
	constexpr view(const Val_T* Ptr, Size_T Count) : Head(Ptr), Size(Count) {}
	//
	constexpr view(view&&) = default;
	//
	constexpr view(const view&) = default;
	//
	constexpr view& operator=(const Val_T* Ptr) {
		return assign(Ptr);
	}
	//
	...
	//
	constexpr view& assign(const Val_T* Ptr, Size_T Count) {
		Head = Ptr;
		Size = Count;
		return *this;
	}
	//
	constexpr view& assign(const Val_T* Ptr) {
		return assign(Ptr, batch::Len<Size_T>(Ptr));
	}
	//
	constexpr view& assign(view&& Ano) {
		return assign(Ano.Head, Ano.Size);
	}
	//
	constexpr view& assign(const view& Ano) {
		return assign(Ano.Head, Ano.Size);
	}
	//
...
void xsl_test_view() {
	...
	TEST_ACT_PRINT(view<char> str00{ cstr00 });
	TEST_RET_PRINT(str00, print);
	TEST_ACT_PRINT(view<char> str01(cstr01, 6));
	TEST_RET_PRINT(str01, print);
	TEST_ACT_PRINT(view<char> str02{ as_rreference(str00) });
	TEST_RET_PRINT(str02, print);
	TEST_ACT_PRINT(view<char> str03{ str01 });
	...
	TEST_ACT_PRINT(view<wchar_t> wstr00{ cwstr00 });
	TEST_RET_PRINT(wstr00, print);
	TEST_ACT_PRINT(view<wchar_t> wstr01(cwstr01, 6));
	TEST_RET_PRINT(wstr01, print);
	TEST_ACT_PRINT(view<wchar_t> wstr02{ as_rreference(wstr00) });
	TEST_RET_PRINT(wstr02, print);
	TEST_ACT_PRINT(view<wchar_t> wstr03{ wstr01 });
	...
	TEST_ACT_PRINT(view<test_t> ttstr00{ test_ts });
	TEST_RET_PRINT(ttstr00, print);
	TEST_ACT_PRINT(view<test_t> ttstr01(test_ts + 3, 3));
	TEST_RET_PRINT(ttstr01, print);
	TEST_ACT_PRINT(view<test_t> ttstr02{ as_rreference(ttstr00) });
	TEST_RET_PRINT(ttstr02, print);
	TEST_ACT_PRINT(view<test_t> ttstr03{ ttstr01 });
	...
	TEST_ACT_PRINT(view<test_p> tpstr00{ test_ps });
	TEST_RET_PRINT(tpstr00, print);
	TEST_ACT_PRINT(view<test_p> tpstr01(test_ps + 3, 3));
	TEST_RET_PRINT(tpstr01, print);
	TEST_ACT_PRINT(view<test_p> tpstr02{ as_rreference(tpstr00) });
	TEST_RET_PRINT(tpstr02, print);
	TEST_ACT_PRINT(view<test_p> tpstr03{ tpstr01 });
