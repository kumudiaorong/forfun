...
namespace xsl::net {
  ...
  using MS2S = xsl::map<String, String, xsl::fc::less<void>>;
  using MI2S = xsl::map<int, String>;
  using MMS2S = xsl::multimap<String, String, xsl::fc::less<void>>;
  using t4sv = xsl::tuple<String_View, String_View, String_View, String_View>;
  template <class F>
  using Func = xsl::fn::function<F>;
  class decompressor {
    z_stream Stream;
    bool Is_valid;
  public:
