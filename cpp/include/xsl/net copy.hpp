...
using Multimap = xsl::multimap<String, String, xsl::fctor::less<void>>;
template <class F>
...
namespace http {
  ...
  xsl::tuple<String, String, String, String> analysis_url(const char *url) {
    std::cmatch cm{};
    std::regex_search(url, cm, Re::url);
    xsl::tuple<String, String, String, String> shp{};
  ...
  public:
    ...
    int req(const char *method, const char *url, MS2S header, const String& body, bool hb = false,
      ...
      String buf{};
      assemble_request_line(method, (ipe ? shp.get<3>() : percent_encode(shp.get<3>())), buf);
      add_default_http_header(header);
      assemble_request_header(header, buf);
      if(hb) assemble_request_body(body, buf);
      std::cout.write(buf.data(), buf.size());
      auto& sfd = *Wait.try_emplace(shp.get<1>().data(), port.data()).first;
      // auto sfd = socket(shp.get<1>().data(), port.data());
      // auto fd = connect(shp.get<1>().data(), port.data());
      // send(fd, buf);
      sfd.send(buf);
      Iom.add(sfd.fd, detail::IOM_EVENTS::IN);
      return sfd.fd;
    }
