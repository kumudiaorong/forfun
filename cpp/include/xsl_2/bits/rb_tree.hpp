...
namespace impl_rb_tree {
  ...
  public:
    ...
    //
    constexpr uint_8 rela(node *Ptr) { return Ptr == Next[2] ? 2 : (Ptr == Next[1] ? 1 : 0); }
  ...
  auto next(node<_Val> *ptr) {
    if(ptr->Next[1] == nullptr) {
      while(
        ptr->Next[2]->Color != impl_rb_tree::color::Null && ptr->Next[2]->rela(ptr) == 1)  // 最终寻找顶层节点,即head
        ptr = ptr->Next[2];
      ptr = ptr->Next[2];
    } else {
      ptr = ptr->Next[1];
  ...
  auto prev(node<_Val> *ptr) {
    if(ptr->Next[0] == nullptr) {
      while(ptr->Next[2]->Color != impl_rb_tree::color::Null && ptr->Next[2]->rela(ptr) == 0)
        ptr = ptr->Next[2];
      ptr = ptr->Next[2];
    } else {
      ptr = ptr->Next[0];
  ...
  public:
    // clang-format off
	typedef itor::bilateral 	            iter_category;
    ...
    constexpr val_type *operator->() const { return &(Ptr->Val); }
    //
    constexpr bool operator==(const iter& Ano) const { return Ptr == Ano.Ptr; }
    //
...
protected:
  ...
  constexpr node_type *insert_node(const null_node_description_type& nndt, node_type *child) {
    ...
    if(nndt.first->Color == impl_rb_tree::color::Red) double_red_adjust(child, nndt.second);
...
private:
  constexpr node_type *set_root(node_type *root) {
    ...
    root->Next[2] = Head;
    root->Color = impl_rb_tree::color::Black;
    return root;
  }
  //
  constexpr void remove_extremum_node(node_type *child, uint_8 Rela) {
    ...
    if(child->Next[Rela] != nullptr) {
      ...
      Parent->Next[CRela]->Color = impl_rb_tree::color::Black;
    } else if(child->Color == impl_rb_tree::color::Black && Parent != Head)
      raise_branch(Parent, CRela);
  }
  //
  constexpr void raise_branch(node_type *GParent, bool PRela) {
    node_type *Parent = GParent->Next[!PRela];
    if(Parent->Color == impl_rb_tree::color::Red) {
      single_rotate(GParent, !PRela);
      Parent->Color = impl_rb_tree::color::Black;
      GParent->Color = impl_rb_tree::color::Red;
      raise_branch(GParent, PRela);
    } else if(Parent->Next[!PRela] != nullptr && Parent->Next[!PRela]->Color == impl_rb_tree::color::Red) {
      single_rotate(GParent, !PRela);
      Parent->Color = GParent->Color;
      Parent->Next[!PRela]->Color = impl_rb_tree::color::Black;
      GParent->Color = impl_rb_tree::color::Black;
    } else if(Parent->Next[PRela] != nullptr && Parent->Next[PRela]->Color == impl_rb_tree::color::Red) {
      single_rotate(Parent, PRela, !PRela);
      single_rotate(GParent, !PRela);
      GParent->Next[2]->Color = GParent->Color;
      GParent->Color = impl_rb_tree::color::Black;
    } else if(GParent->Color == impl_rb_tree::color::Red) {
      GParent->Color = impl_rb_tree::color::Black;
      Parent->Color = impl_rb_tree::color::Red;
    } else {
      Parent->Color = impl_rb_tree::color::Red;
  ...
  constexpr void double_red_adjust(node_type *child, uint_8 CRtr) {
    ...
    GParent->Color = impl_rb_tree::color::Red;
    if(Uncle == nullptr || Uncle->Color == impl_rb_tree::color::Black) {  // Scale is about n1:n2:1,n1:n2==1:1,n1 is
      ...
      Uncle->Color = impl_rb_tree::color::Black;
      if(GParent->Next[2]->Color == impl_rb_tree::color::Red) double_red_adjust(GParent);
    }
    Parent->Color = impl_rb_tree::color::Black;
  }
